...
local host_alert_snmp_flood = classes.class(alert)

-- ##############################################

host_alert_snmp_flood.meta = {
  alert_key = host_alert_keys.host_alert_snmp_flood,
...
function host_alert_snmp_flood:init(metric, value, operator, threshold)
...
function host_alert_snmp_flood.format(ifid, alert, alert_type_params)
  local alert_consts = require "alert_consts"
  local entity = alert_consts.formatHostAlert(ifid, alert["ip"], alert["vlan_id"])
  local i18n_key

  if alert_type_params.is_attacker then
    i18n_key = "alert_messages.snmp_flood_attacker"
  else
    i18n_key = "alert_messages.snmp_flood_victim"
  end
...
-- @param alert_type_params Table `alert_type_params` as built in the `:init` method
-- @return A human-readable string
function host_alert_snmp_flood.filter_to_past_flows(ifid, alert, alert_type_params)
   local res = {}
