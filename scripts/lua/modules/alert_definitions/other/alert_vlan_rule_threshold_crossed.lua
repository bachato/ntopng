...
local alert_vlan_rule_threshold_crossed = classes.class(alert)

alert_vlan_rule_threshold_crossed.meta = {
  alert_key = other_alert_keys.alert_vlan_rule_threshold_crossed,
...
function alert_vlan_rule_threshold_crossed:init(ifid, vlan_id ,metric, frequency, threshold, value, threshold_sign, metric_type, vlan_label)
...
function alert_vlan_rule_threshold_crossed.format(ifid, alert, alert_type_params)
   ...
   if(alert_type_params.metric == "vlan:traffic" or  alert_type_params.metric == "host_pool:traffic-RX" or alert_type_params.metric == "host_pool:traffic-TX") then
    ...
    return(i18n("alert_messages.traffic_vlan_volume_alert", {
        url = ntop.getHttpPrefix() .. "/lua/hosts_stats.lua?vlan=",
        vlan_id = alert_type_params.vlan_id,
        vlan = alert_type_params.vlan_label,
...
return alert_vlan_rule_threshold_crossed
