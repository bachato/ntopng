...
local function check_network_issues(params)
    ...
    local tot_packets = params.entity_info.inner + params.entity_info.egress +
                            params.entity_info.ingress
    ...
    if params.entity_info["tcpPacketStats.inner"] then
        ...
        tot_retransmissions = tot_retransmissions +
                                  (params.entity_info["tcpPacketStats.inner"]
                                      .retransmissions or 0)
        tot_lost = tot_lost +
                       (params.entity_info["tcpPacketStats.inner"].lost or 0)
        tot_out_of_order = tot_out_of_order +
                               (params.entity_info["tcpPacketStats.inner"]
                                   .out_of_order or 0)
    ...
    if params.entity_info["tcpPacketStats.egress"] then
        ...
        tot_retransmissions = tot_retransmissions +
                                  (params.entity_info["tcpPacketStats.egress"]
                                      .retransmissions or 0)
        tot_lost = tot_lost +
                       (params.entity_info["tcpPacketStats.egress"].lost or 0)
        tot_out_of_order = tot_out_of_order +
                               (params.entity_info["tcpPacketStats.egress"]
                                   .out_of_order or 0)
    ...
    if params.entity_info["tcpPacketStats.ingress"] then
        ...
        tot_retransmissions = tot_retransmissions +
                                  (params.entity_info["tcpPacketStats.ingress"]
                                      .retransmissions or 0)
        tot_lost = tot_lost +
                       (params.entity_info["tcpPacketStats.ingress"].lost or 0)
        tot_out_of_order = tot_out_of_order +
                               (params.entity_info["tcpPacketStats.ingress"]
                                   .out_of_order or 0)
    ...
    local retransmissions_threshold = (params.check_config.retransmissions
                                          .threshold or
                                          params.check_config.retransmissions
                                              .default_value)
    local out_of_orders_threshold =
        (params.check_config.out_of_orders.threshold or
            params.check_config.out_of_orders.default_value)
    local lost_threshold = (params.check_config.packet_loss.threshold or
                               params.check_config.packet_loss.default_value)
    ...
    -- istantiate alert
    local alert = alert_consts.alert_types.alert_network_issues.new(lost,
                                                                    lost_threshold,
                                                                    retransmissions,
    ...
    if (lost > lost_threshold or retransmissions > retransmissions_threshold or
