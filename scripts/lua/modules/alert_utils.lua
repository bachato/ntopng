...
function alert_utils.getConfigsetURL(script_key, subdir)
   return
      string.format('%s/lua/admin/edit_configset.lua?subdir=%s&check=%s#all', ntop.getHttpPrefix(), subdir or "all", script_key or "all")
end
   ...
   if isAdministrator() then
      ...
      if alert_entity and alert_entity == alert_entities.am_host.entity_id then
         ...
         if host then
            ...
            if measurement then
               return
                  ' <a href="' .. ntop.getHttpPrefix() .. '/lua/active_monitoring.lua?host=' .. host .. '&measurement=' .. measurement ..
                     '&page=overview"><i class="fas fa-cog" title="' .. i18n("edit_configuration") .. '"></i></a>'
            else
               return ' <a href="' .. ntop.getHttpPrefix() .. '/lua/active_monitoring.lua?host=' .. host ..
                         '&page=overview"><i class="fas fa-cog" title="' .. i18n("edit_configuration") .. '"></i></a>'
            end
         else
            return ' <a href="' .. ntop.getHttpPrefix() .. '/lua/active_monitoring.lua?page=overview"><i class="fas fa-cog" title="' ..
...
function alert_utils.getLinkToPastFlows(ifid, alert, alert_json)
   ...
   if alert_id and epoch_begin and epoch_end then
      ...
      local res = string.format("%s/lua/pro/db_search.lua?epoch_begin=%u&epoch_end=%u&%s", ntop.getHttpPrefix(), epoch_begin, epoch_end,
...
local function notify_ntopng_status(started)
   ...
   local score = 10
   local msg
   local msg_details = string.format("%s v.%s (%s) [OS: %s][pid: %s][options: %s]", info.product, info.version, info.revision, info.OS,
      info.pid, info.command_line)
...
local function addDayEpoch(res, day_epoch)
   res[day_epoch] = {0, --[[ Counter for alerts between 00:00 and 00:59 UTC --]] 0, --[[ Counter for alerts between 01:00 and 01:59 UTC --]]
               ...
               if has_bit then -- The bit is set
                  ...
                  if alert_id ~= tonumber(predominant_alert) then -- Do not add the predominant alert to the list of additional alerts
                     ...
                     if alert_risk > 0 then -- source is nDPI
                        if not no_html then
                           message = string.format("%s %s", message, flow_risk_utils.get_remediation_documentation_link(alert_risk, "nDPI"))
                        end
                        local info_msg = alert_utils.get_flow_risk_info(alert_risk, alert_json)
