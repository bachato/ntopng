...
function syslog_utils.handle_event(message, host, priority, level_threshold)
    ...
    if level_threshold and level <= level_threshold then
        ...
        local entity = alert_entity_builders.hostAlertEntity(host, 0)
        ...
        local type_info = alert_consts.alert_types.host_alert_host_log.new(host, level_name, facility_name, message)
...
function syslog_utils.handle_system_event(host, service, event, message, priority, level_threshold)
    ...
    if level_threshold and level <= level_threshold then
        ...
        local entity_info = alert_entity_builders.systemEntity(entity_value)

        local type_info = alert_consts.alert_types.alert_process_notification.new(event, message)
