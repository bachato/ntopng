...
function ts_dump.iface_update_ndpi_rrds(when, _ifname, ifstats, verbose, config)
    ...
    for k in pairs(ifstats["ndpi"]) do
        ...
        if config.ndpi_flows_timeseries_creation == "1" then
            ts_utils.append("iface:ndpi_flows", {
                ifid = ifstats.id,
                protocol = k,
                num_flows = ifstats["ndpi"][k]["num_flows"]
            }, when)
        end
    end
end
...
function ts_dump.iface_update_stats_rrds(when, _ifname, ifstats, verbose)
    ...
    if (ifstats["localstats"]["bytes"]["local2remote"] > 0) then
        ts_utils.append("iface:local2remote", {
            ifid = ifstats.id,
            bytes = ifstats["localstats"]["bytes"]["local2remote"]
        }, when)
    end

    if (ifstats["localstats"]["bytes"]["remote2local"] > 0) then
        ts_utils.append("iface:remote2local", {
            ifid = ifstats.id,
            bytes = ifstats["localstats"]["bytes"]["remote2local"]
        }, when)
    end
end
    ...
    for subnet, sstats in pairs(subnet_stats or {}) do
        ...
        if not ifstats.isViewed and not ifstats.isView then
            ts_utils.append("subnet:rtt", {
                ifid = ifstats.id,
                subnet = subnet,
                millis_rtt = sstats["round_trip_time"]
            }, when)
        end
        ...
        ts_utils.append("subnet:traffic", {
            ifid = ifstats.id,
            subnet = subnet,
            bytes_ingress = sstats["ingress"],
            bytes_egress = sstats["egress"],
            bytes_inner = sstats["inner"]
        }, when)

        ts_utils.append("subnet:broadcast_traffic", {
            ifid = ifstats.id,
            subnet = subnet,
            bytes_ingress = sstats["broadcast"]["ingress"],
            bytes_egress = sstats["broadcast"]["egress"],
            bytes_inner = sstats["broadcast"]["inner"]
        }, when)
        ...
        ts_utils.append("subnet:score", {
            ...
            subnet = subnet,
            score = sstats["score"],
            scoreAsClient = sstats["score.as_client"],
        ...
        if not ifstats.isSampledTraffic then
            ts_utils.append("subnet:tcp_retransmissions", {
                ...
                packets_inner = sstats["tcpPacketStats.inner"]["retransmissions"]
            }, when)
            ...
            ts_utils.append("subnet:tcp_out_of_order", {
                ...
                packets_inner = sstats["tcpPacketStats.inner"]["out_of_order"]
            }, when)
            ...
            ts_utils.append("subnet:tcp_lost", {
                ...
                packets_inner = sstats["tcpPacketStats.inner"]["lost"]
            }, when)
            ...
            ts_utils.append("subnet:tcp_keep_alive", {
                ...
                packets_inner = sstats["tcpPacketStats.inner"]["keep_alive"]
            }, when)
        end
...
function ts_dump.iface_update_general_stats(when, ifstats, verbose)
    -- Score
    ts_utils.append("iface:score", {
        ifid = ifstats.id,
        srv_score = ifstats.score.score_as_srv,
        cli_score = ifstats.score.score_as_cli
    }, when)
    ...
    if not ifstats.isViewed then
        ...
        ts_utils.append("iface:http_hosts", {
            ifid = ifstats.id,
    ...
    if ntop.isEnterpriseM() then
        ...
        ts_utils.append("iface:score_behavior_v2", {
            ...
            upper_bound = ifstats["score_behavior"]["upper_bound"]
        }, when)
        ...
        ts_utils.append("iface:traffic_tx_behavior_v5", {
            ...
            upper_bound = ifstats["traffic_tx_behavior"]["upper_bound"]
        }, when)
...
function ts_dump.update_internals_periodic_activities_stats(when, ifstats,
    ...
    for ps_name, ps_stats in pairs(periodic_scripts_stats) do
        ...
        ts_utils.append("periodic_script:duration", {
            ifid = ifstats.id,
            periodic_script = ps_name,
            num_ms_last = num_ms_last
        }, when)
        ...
        if ts_utils.getDriverName() == "rrd" then
            if ps_stats["timeseries"] and ps_stats["timeseries"]["write"] then
                ...
                if tot_calls + tot_drops > 0 then
                    ts_utils.append("periodic_script:timeseries_writes", {
                        ifid = ifstats.id,
                        periodic_script = ps_name,
                        writes = tot_calls,
                        drops = tot_drops
...
function ts_dump.containers_update_stats(when, ifstats, verbose)
    ...
    for container_id, container in pairs(containers_stats) do
        ts_utils.append("container:num_flows", {
            ifid = ifstats.id,
            container = container_id,
            as_client = container["num_flows.as_client"],
            as_server = container["num_flows.as_server"]
        }, when)

        ts_utils.append("container:rtt", {
            ifid = ifstats.id,
            container = container_id,
            as_client = container["rtt_as_client"],
            as_server = container["rtt_as_server"]
        }, when)

        ts_utils.append("container:rtt_variance", {
            ifid = ifstats.id,
            container = container_id,
            as_client = container["rtt_variance_as_client"],
            as_server = container["rtt_variance_as_server"]
        }, when)
    end
end
...
function ts_dump.pods_update_stats(when, ifstats, verbose)
    ...
    for pod_id, pod in pairs(pods_stats) do
        ts_utils.append("pod:num_containers", {
            ifid = ifstats.id,
            pod = pod_id,
            num_containers = pod["num_containers"]
        }, when)

        ts_utils.append("pod:num_flows", {
            ifid = ifstats.id,
            pod = pod_id,
            as_client = pod["num_flows.as_client"],
            as_server = pod["num_flows.as_server"]
        }, when)

        ts_utils.append("pod:rtt", {
            ifid = ifstats.id,
            pod = pod_id,
            as_client = pod["rtt_as_client"],
            as_server = pod["rtt_as_server"]
        }, when)

        ts_utils.append("pod:rtt_variance", {
            ifid = ifstats.id,
            pod = pod_id,
            as_client = pod["rtt_variance_as_client"],
            as_server = pod["rtt_variance_as_server"]
        }, when)
    end
end
...
function ts_dump.iface_update_anomalies(when, ifstats, verbose)
    if not ifstats.isViewed then
        ts_utils.append("iface:hosts_anomalies", {
            ifid = ifstats.id,
            num_loc_hosts_anom = ifstats.anomalies.num_local_hosts_anomalies,
            num_rem_hosts_anom = ifstats.anomalies.num_remote_hosts_anomalies
        }, when)
    end
end
