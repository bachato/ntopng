...
const ntopChartApex = function() {
  ...
  const _default_TS_COLUMN_ChartOptions = function() {
    const chartOptions = ntopng_utility.clone(_default_BASE_ChartOptions);
    const TS_COLUMN_ChartOptions = {
      chart: {
        stacked: true,
        type: 'bar',
        zoom: {
    ...
    newChart: function(type) {
      ...
      const setYaxisFormatter = (chartOptions) => {
        if (typeof (chartOptions?.yaxis?.labels?.formatter) == 'string') {
          ...
          const chartFormatter = FormatterUtils.getFormatter(formatter);
      ...
      return {
        ...
        to_data_uri: async function(options) {
          ...
          const res = await _chart.dataURI(options);
          return res.imgURI;
        },
