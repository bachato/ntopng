...
const sources_url_el_to_source = {
  ...
  am_host: (am) => {
    const label = `${am.label} ${am.measurement}`;
    const value = `${am.host},metric:${am.measurement_key}`;
    return {
      label,
      value,
    };
  ...
  },
  {
    ...
    source_def_array: [{
      ...
      sources_function: async (selected_source_value_array) => {
        ...
        const snmp_interfaces = await proxySnmp.available_interfaces(device_host);
        return snmp_interfaces.map((iface) => {
          return {label: iface.name, value: iface.id};
        });
      },
      value: 'if_index',
