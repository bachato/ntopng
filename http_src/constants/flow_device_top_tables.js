...
const top_flow_interface = {
  ...
  f_get_label: (ts_group) => {
    const source_def_array = ts_group.source_type.source_def_array;
    let source_label;
    for (let i = 0; i < source_def_array.length; i += 1) {
      if (source_def_array[i].value != 'device') {
        continue;
      }
