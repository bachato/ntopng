...
const top_application = {
  ...
  columns: [{
    ...
    render_if: function(context) {
      return context.is_history_enabled;
    },
    render: function(data, type, service) {
      ...
      const jump_to_historical = {
        ...
        onClick: function() {
          const l7_proto = ntopng_url_manager.serialize_param('l7proto', `${service.protocol.id};eq`);
          ...
          const source_array = context.source_array;

          let params = '';
          const params_array = source_type.source_def_array.map((source_def, i) => {
      ...
      const jump_to_live = {
        ...
        onClick: function() {
          const l7_proto = ntopng_url_manager.serialize_param('application', `${service.protocol.id}`);
    ...
    render: function(data, type, service) {
      ...
      const handler = {
        ...
        onClick: function() {
          // console.log(data);
          // console.log(service);
          const schema = `top:${service.ts_schema}`;
  ...
  columns: [{
    ...
    render: function(data, type, service) {
      ...
      const jump_to_historical = {
        handlerId: handlerIdJumpHistorical,
        onClick: function() {
          const category = ntopng_url_manager.serialize_param('l7cat', `${service.category.id};eq`);
      ...
      const jump_to_live = {
        handlerId: handlerIdJumpLive,
        onClick: function() {
          const l7_category = ntopng_url_manager.serialize_param('application', `cat_${service.category.id}`);
      ...
      render: function(data, type, service) {
        ...
        const jump_to_historical = {
          ...
          onClick: function() {
            ...
            let params = '';
            const params_array = [];
            for (const key in service.tags) {
              const value = service.tags[key];
              let p_url = '';
              if (key == 'ifid') {
