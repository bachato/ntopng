...
<template>
    <modal :id="id_modal" ref="modal">
        ...
        <template v-slot:body>
            <form autocomplete="off">
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <select-search v-model:selected_option="filter_type_selected" :id="'filter_type'"
                            :options="filters_options" @select_option="change_filter()">
                        </select-search>
                    </div>
                </div>
                <hr>
                    ...
                    <div class="col-sm-8">
                        <div class="input-group mb-3">
                            <div class="input-group-prepend col-sm-3">
                                <select-search v-model:selected_option="operator_selected" :id="'operator_filter'"
                                    :options="operators_to_show" @select_option="change_operator_type">
                                </select-search>
                            </div>
                            ...
                            <template v-else>
                                <input v-model="input_value"
                                    ...
                                    :pattern="data_pattern_selected" name="value" :required="input_required" type="text"
                                    class="form-control">
                                <span style="margin: 0px;padding:0;"
                                    class="alert invalid-feedback">{{ i18n('invalid_value') }}</span>
                            </template>
                        </div>
        ...
        <template v-slot:footer>
            <button type="button" :disabled="check_disable_apply()" @click="apply"
                class="btn btn-primary">{{ i18n('apply') }}</button>
        </template>
    </modal>
</template>
