    ...
    <div class="button-group mb-2 d-flex align-items-center">
        <template v-for="(value, index) in page_filters">
            <div v-if="value.length > 0" class="dropdown me-2 d-flex"><span
                    class="no-wrap d-flex align-items-center filters-label me-2"><b>{{ _i18n('server_ports.' +
                        page_filters_title[index]) }}:
                    </b></span>
                <SelectSearch v-model:selected_option="active_filter_list[index]" :options="value"
    ...
    for (const [name, filters] of Object.entries(props.context.available_filters)) {
        filters.forEach((filter) => {
            ...
            if (filter.currently_active)
                active_filter_list.value.push(filter);
        })
        page_filters.value.push(filters);
        page_filters_title.value.push(name);
        if (!active_filter_list.value[index]) {
            active_filter_list.value[index] = filters[0]; /* No filter, use the first one */
        }
        index = index + 1;
    }
});
...
const changedOption = (opt) => {
    if (opt.filter_name === "timeframe" && opt.id !== 'none') {
        current_rest = historical_rest
    } else {
        current_rest = live_rest
    }
