...
const DataTableHandlers = function() {
  ...
  return {
    ...
    getHandler: function(handlerId, rowId) {
      const handlers = handlersIdDict[handlerId];
      if (handlers == null) {
        return null;
      }
...
window['_DataTableButtonsOnClick'] = function(handlerId, rowId) {
  const onClick = DataTableHandlers.getHandler(handlerId, rowId);
  if (onClick != null) {
    onClick();
  }
};
...
export class DataTableUtils {
  ...
  static createLinkCallback(action) {
    let handler = '';
    const fOnClick = DataTableHandlers.addHandler(action.handler);
...
export class DataTableRenders {
  ...
  static hideIfZero(obj, type, row, zero_is_null) {
    ...
    const value = (obj.value !== undefined ? obj.value : obj);
  ...
  static filterize(key, value, label, tag_label, title, html, is_snmp_ip, ip) {
    ...
    if (is_snmp_ip != null) {
      if (is_snmp_ip) {
        if (value) {
          const url = NtopUtils.buildURL(`${http_prefix}/lua/pro/enterprise/snmp_device_details.lua?host=${value}`);
        ...
        if (ip && value) {
          ...
          const url = NtopUtils.buildURL(`${http_prefix}/lua/pro/enterprise/snmp_interface_details.lua?host=${ip}&snmp_port_idx=${value}`);
          content += ` <a href='${url}' data-bs-toggle='tooltip' data-bs-placement="bottom" title=''><i class='fas fa-laptop'></i></a>`;
        }
      }
    }
  ...
  static formatSRC2DSTTCPFlags(obj, type, row, zero_is_null) {
    if (type !== 'display') return obj.value;
    const cell = `<a class='tag-filter' data-tag-key='src2dst_tcp_flags' data-tag-value='${obj.value}' data-tag-label='${obj.label}' href='javascript:void(0)'>${obj.label}</a>`;
    return cell;
  }

  static formatDST2SRCTCPFlags(obj, type, row, zero_is_null) {
    if (type !== 'display') return obj.value;
    const cell = `<a class='tag-filter' data-tag-key='dst2src_tcp_flags' data-tag-value='${obj.value}' data-tag-label='${obj.label}' href='javascript:void(0)'>${obj.label}</a>`;
    return cell;
  }
  ...
  static formatSubtype(obj, type, row, zero_is_null) {
    ...
    const label = DataTableRenders.filterize('subtype', obj, obj);

    return label;
  }

  static filterize_2(key, value, label, tag_label, title, html) {
    if (value == null || (value == 0 && (label == null || label == ''))) {
      return '';
    }
  ...
  static formatSNMPInterface(obj, type, row) {
    ...
    if (obj.value === undefined || obj.value == '') {
      return '';
    } /* interface not defined - probably a device-level alert */
  ...
  static formatProbeIP(obj, type, row, zero_is_null) {
    ...
    const label = DataTableRenders.filterize('probe_ip', obj.value, obj.label, obj.label, obj.label_long);
  ...
  static formatIP(ip, type, row, zero_is_null) {
    ...
    const label = DataTableRenders.filterize(ip.tag_key, ip.value, ip.label, ip.label, title);
    return label;
  }

  static formatFlowTuple(flow, type, row, zero_is_null) {
    const active_ref = (flow.active_url ? `<a href="${flow.active_url}"><i class="fas fa-stream"></i></a>` : '');
    let cliLabel = '';
    if (flow.cli_ip.name) {
      let title = '';
    ...
    const cliPortLabel = ((flow.cli_port && flow.cli_port.value && flow.cli_port.value != 0) ? ':' + DataTableRenders.filterize('cli_port', flow.cli_port.value, flow.cli_port.label) : '');
    ...
    const srvPortLabel = ((flow.srv_port && flow.srv_port.value && flow.srv_port.value != 0) ? ':' + DataTableRenders.filterize('srv_port', flow.srv_port.value, flow.srv_port.label) : '');
  ...
  static formatSubtypeValueLabel(obj, type, row, zero_is_null) {
    ...
    const msg = DataTableRenders.filterize('subtype', obj.value, obj.name, obj.fullname, obj.fullname);
  ...
  static formatNameDescription(obj, type, row, zero_is_null) {
    if (type !== 'display') return obj.name;
    const msg = DataTableRenders.filterize('alert_id', obj.value, obj.name, obj.fullname, obj.fullname);
