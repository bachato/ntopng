export function draw_processes_graph(http_prefix, graph_div_id, host) {
  ...
  const url = http_prefix + '/lua/get_processes_graph_data.lua?host=' + host;
  ...
  d3.json(url, function(error, json) {
    ...
    const width = 960; const height = 500; const arrow_size = 6;
    ...
    const svg = d3.select('#' + graph_div_id).append('svg')
	    .attr('id', 'ebpf_graph')
	    .attr('width', width)
	    .attr('height', height);
