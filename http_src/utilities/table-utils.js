...
async function build_table(http_prefix, table_id, f_map_columns, f_get_extra_params_obj, f_on_get_rows) {
  const epoch_end = Number.parseInt(Date.now() / 1000);
  const table_def_url = `${http_prefix}/tables_config/${table_id}.json?epoch_end=${epoch_end}`;
  const table_def = await ntopng_utility.http_request(table_def_url, null, null, true);
  if (table_def == null) {
    return {
      id: table_id,
      columns: [],
  ...
  if (table_def.columns) {
    table_def.columns.forEach((item, x) => {
      if (item.button_def_array) {
        item.button_def_array.forEach((button, y) => {
          if (!button.class) {
            button.class = [];
          }
        });
...
function get_f_print_vue_node_row(table_def) {
  ...
  return (col, row, vue_obj, return_true_if_def) => {
    if (col.render_v_node_type == null && col.render_v_func == null) {
      return null;
    }
    if (return_true_if_def == true) {
      return true;
    }
    if (col.render_v_func != null) {
      return col.render_v_func(col, row, vue_obj, return_true_if_def);
    }
    const print_v_node_buttons = available_types[col.render_v_node_type];
...
function get_f_print_html_row(table_def) {
  return (col, row, return_true_if_def) => {
    ...
    if (col.render_generic != null) {
      const render = DataTableRenders.getFormatGenericField(col.render_generic, col.zero_is_null);
...
function get_f_print_v_node_buttons(list_or_array) {
  return (col, row, vue_obj) => {
    const on_click = (id) => {
      return (e) => {
        e.stopPropagation();
        const event = {event_id: id, row, col};
        vue_obj.emit('custom_event', event);
      };
    };
    const v_nodes = col.button_def_array.map((b_def) => {
      let attributes = {};
    ...
    if (list_or_array == true) {
      const v_title = vue_obj.h('span', {class: 'fas fa-sm fa-align-justify'});
      const dropdown = vue_obj.h(Dropdown, {auto_load: true, button_style: '', button_class: 'btn-secondary btn-sm'}, {
        title: () => v_title,
        menu: () => v_nodes,
      });
      return dropdown;
...
function get_rows_func(table_def, f_get_extra_params_obj, f_on_get_rows) {
  const f_get_column_id = get_column_id_func(table_def);
  return async (active_page, per_page, columns_wrap, map_search, first_get_rows) => {
    const sort_column = columns_wrap.find((c) => c.sort != 0);
    const visible_columns = columns_wrap.filter((c) => c.visible == true)
    ...
    if (f_get_extra_params_obj != null) {
      const extra_params = f_get_extra_params_obj();
...
function get_columns_from_datatable_columns(datatable_columns_title, datatable_columns_js) {
  const table_columns_config = datatable_columns_js.map((c, index) => {
    const config = {
      data_field: c.data,
