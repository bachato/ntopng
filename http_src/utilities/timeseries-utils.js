...
function tsArrayToOptionsArray(tsOptionsArray, tsGroupsArray, groupsOptionsMode, tsCompare) {
  ...
  const splittedTsArray = splitTsArrayStacked(tsOptionsArray, tsGroupsArray);
  const DygraphOptionsStacked = tsArrayToOptionsArrayRaw(splittedTsArray.stacked.tsOptionsArray, splittedTsArray.stacked.tsGroupsArray, groupsOptionsMode, tsCompare);
  const DygraphOptionsNotStacked = tsArrayToOptionsArrayRaw(splittedTsArray.not_stacked.tsOptionsArray, splittedTsArray.not_stacked.tsGroupsArray, groupsOptionsMode, tsCompare);
...
function splitTsArrayStacked(tsOptionsArray, tsGroupsArray) {
  const tsOptionsArrayStacked = [];
  const tsGroupsArrayStacked = [];
  const tsOptionsArrayNotStacked = [];
  const tsGroupsArrayNotStacked = [];
...
function tsArrayToOptionsArrayRaw(tsOptionsArray, tsGroupsArray, groupsOptionsMode, tsCompare) {
  ...
  if (groupsOptionsMode.value == groupsOptionsModesEnum['1_chart_x_yaxis'].value) {
    const tsDict = {};
    tsGroupsArray.forEach((tsGroup, i) => {
      const yaxisId = getYaxisId(tsGroup.metric);
      const tsEl = {tsGroup, tsOptions: tsOptionsArray[i]};
    ...
    const DygraphOptionsArray = [];
    for (const key in tsDict) {
      const tsArray = tsDict[key];
      const tsOptionsArray2 = tsArray.map((ts) => ts.tsOptions);
      const tsGroupsArray2 = tsArray.map((ts) => ts.tsGroup);
      const DygraphOptions = tsArrayToOptions(tsOptionsArray2, tsGroupsArray2, tsCompare, useFullName);
    ...
    tsOptionsArray.forEach((tsOptions, i) => {
      const options = tsArrayToOptions([tsOptions], [tsGroupsArray[i]], tsCompare, useFullName);
...
function getTsQuery(tsGroup, not_metric_query, enable_source_def_value_dict) {
  let tsQuery = tsGroup.source_type.source_def_array.map((source_def, i) => {
    if (enable_source_def_value_dict != null && !enable_source_def_value_dict[source_def.value]) {
      return null;
    }
    const source_value = tsGroup.source_array[i].value;
...
function getTsChartsDataUrl(httpPrefix, epochStatus, tsCompare, timeseriesGroups, isPro) {
  const paramsEpochObj = {epoch_begin: epochStatus.epoch_begin, epoch_end: epochStatus.epoch_end};
  const url_and_body = {};
  if (!isPro) {
    const tsDataUrl = `${httpPrefix}/lua/rest/v2/get/timeseries/ts.lua`;
    const paramsUrlRequest = `ts_compare=${tsCompare}&version=4&zoom=${tsCompare}&limit=180`;
    const tsGroup = timeseriesGroups[0];
    ...
    const pUrlRequest = ntopng_url_manager.add_obj_to_url(pObj, paramsUrlRequest);
    ...
    const tsRequests = timeseriesGroups.map((tsGroup) => {
      const main_source_index = getMainSourceDefIndex(tsGroup);
      const tsQuery = getTsQuery(tsGroup);
      const pObj = {
        ts_query: tsQuery,
...
async function getTsChartsOptions(httpPrefix, epochStatus, tsCompare, timeseriesGroups, isPro) {
  ...
  if (!isPro) {
    const tsChartOption = await ntopng_utility.http_request(url_and_body.url);
