...
const get_num_open_ports_icon = (diff_case, unused_port_list, filtered_port_list) => {
  ...
  switch (diff_case) {
    case 4: {
      const unused_port_list_string = ports_list_string(unused_port_list);
      label = ` <span class="badge bg-secondary"><i class="fa-solid fa-ghost" title='${unused_port_list_string}'></i></span></div>`;
    }
      break;
    case 3: {
      const filtered_ports_list_string = ports_list_string(filtered_port_list);
...
const get_icon_component = (item, row, ports_unused, ports_fitered) => {
  const port = item.split(' ')[0].split('/')[0];
  const port_badge = find_badge(Number(port), row, ports_unused, ports_fitered);
  let icon_comp = null;
  switch (port_badge) {
    case 'unused':
...
export const hosts_f = (hosts, row) => {
  ...
  hosts_map.forEach((values, keys) => {
    const url = build_host_to_scan_report_url(values.ip, values.scan_type, values.date, values.epoch);
...
export const host_f = (host, row, ifid) => {
  let label = host;
  const host_not_reachable = row.is_ok_last_scan == 5 && row.is_down != null && row.is_down == true;
  if ((row.is_ok_last_scan == 1 || host_not_reachable) && (row.last_scan != null && row.last_scan.time != null)) {
    const url = build_host_to_scan_report_url(host, row.scan_type, row.last_scan.time.replace(' ', '_'), row.last_scan.epoch);
