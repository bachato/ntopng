...
export const ntopng_utility = function() {
  ...
  return {
    ...
    get_timeframes_dict: function() {
      ...
      const t_week = new Date();
      const t_month = new Date();
      const t_year = new Date();
      return {
        'min': min,
        '5_min': min * 5,
        '10_min': min * 10,
    ...
    get_timeframe_from_timeframe_id: function(timeframe_id) {
      const timeframes_dict = this.get_timeframes_dict();
    ...
    set_default_time_interval: function(time_interval_id='30_min', round_timeframe_id) {
      ...
      const seconds_in_interval = this.get_timeframe_from_timeframe_id(time_interval_id);
    ...
    from_utc_s_to_server_date: function(utc_seconds) {
	    const utc = utc_seconds * 1000;
	    const d_local = new Date(utc);
	    const local_offset = d_local.getTimezoneOffset();
	    const server_offset = moment.tz(utc, ntop_zoneinfo)._offset;
	    const offset_minutes = server_offset + local_offset;
	    const offset_ms = offset_minutes * 1000 * 60;
	    const d_server = new Date(utc + offset_ms);
	    return d_server;
    },
    get_date_format: async function(is_range_picker, csrf, http_prefix) {
      ...
      const date_format_url = `${http_prefix}/lua/rest/v2/get/timeseries/date_format.lua`;
    ...
    from_utc_to_server_date_format: function(utc_ms, format) {
	    ...
	    const m = moment.tz(utc_ms, ntop_zoneinfo);
	    const tz_server = m.format(format);
	    return tz_server;
    },
...
export const ntopng_status_manager = function() {
  ...
  const subscribers = {}; // dictionary of { [id: string]: f_on_ntopng_status_change() }
  ...
  const notify_subscribers = function(status, skip_id) {
    for (const id in subscribers) {
      ...
      const f_on_change = subscribers[id];
  ...
  return {
    ...
    add_obj_to_status: function(obj, skip_id) {
      const new_status = this.get_status();
...
export const ntopng_url_manager = function() {
  ...
  const custom_params_serializer = {};
  ...
  return {
    ...
    get_url_entry: function(param_name, url) {
      const entries = this.get_url_entries(url);
      for (const [key, value] of entries) {
        if (key == param_name) {
          return value;
        }
    ...
    obj_to_url_params: function(obj) {
      ...
      for (const key in obj) {
        ...
        const param = serializer(key, obj[key]);
        params.push(param);
      }
    ...
    delete_params: function(params_key) {
      const search_params = this.get_url_search_params();
    ...
    add_obj_to_url: function(url_params_obj, url) {
      const new_url_params = this.obj_to_url_params(url_params_obj);
      const search_params = this.get_url_search_params(url);
      const new_entries = this.get_url_entries(new_url_params);
      for (const [key, value] of new_entries) {
        	search_params.set(key, value);
      }
      const new_url = search_params.toString();
      if (url != null) {
        return new_url;
      }
...
export const ntopng_events_manager = function() {
  ...
  const events_subscribers = {}; // dictionary of { [event_name: string]: { [id: string]: f_on_event }
  ...
  const on_status_change = function(new_status) {
    for (const event_name in ntopng_events) {
      const f_compare = ntopng_events_compare[event_name];
      if (f_compare(new_status, status) == true) {
        const subscribers = events_subscribers[event_name];
  ...
  const emit = function(event, params, skip_id, dest_id) {
    ...
    const subscribers = events_subscribers[event];
    if (subscribers == null) {
      return;
    }
