...
public:
  ...
  SiteFlowStats(u_int64_t s = 0, u_int64_t r = 0) { sent = s, rcvd = r; }
};

static bool aggregate_site_flows(GenericHashEntry *node, void *user_data,
				 bool *matched) {
  Flow *f = (Flow *)node;
  std::map<u_int64_t, SiteFlowStats> *agg_stats = static_cast< std::map<u_int64_t, SiteFlowStats>* >(user_data);
  u_int64_t key;
  u_int32_t ip_src, ip_dst;
  bool swapped;

  ip_src = f->getFlowDeviceIP();
  ip_dst = f->getFlowDeviceNextHop();

  if(ip_src < ip_dst)
    key = (((u_int64_t)ip_src) << 32) + (u_int64_t)ip_dst, swapped = false;
  else
    key = (((u_int64_t)ip_dst) << 32) + (u_int64_t)ip_src, swapped = true;

  std::map<u_int64_t, SiteFlowStats>::iterator it = agg_stats->find(key);

  if(it == agg_stats->end()) {
    /* Not found: let's add it */

    if(!swapped)
      agg_stats->insert(std::make_pair(key, SiteFlowStats(f->get_bytes_cli2srv(), f->get_bytes_srv2cli())));
    else
      agg_stats->insert(std::make_pair(key, SiteFlowStats(f->get_bytes_srv2cli(), f->get_bytes_cli2srv())));
  } else {
    /* Found let's update it */

    if(!swapped)
      it->second.sent += f->get_bytes_cli2srv(), it->second.rcvd += f->get_bytes_srv2cli();
    else
      it->second.sent += f->get_bytes_srv2cli(), it->second.rcvd += f->get_bytes_cli2srv();
  }

  return (false); /* false = keep on walking */
}

bool NetworkInterface::aggregateSiteFlows(lua_State *vm) {
  u_int32_t i = 0, begin_slot = 0;
  std::map<u_int64_t, SiteFlowStats> agg_flows;
  std::map<u_int64_t, SiteFlowStats>::iterator it;

  walker(&begin_slot, true /* walk_all */, walker_flows, aggregate_site_flows, &agg_flows);

  lua_newtable(vm);

  for(it = agg_flows.begin(); it != agg_flows.end(); it++) {
    u_int64_t key = it->first;
    SiteFlowStats *v = &(it->second);
    char buf[32];
    u_int32_t ip_src = (key >> 32) & 0x00000000FFFFFFFF;
    u_int32_t ip_dst = key & 0x00000000FFFFFFFF;

    lua_newtable(vm);
    lua_push_str_table_entry(vm, "src", Utils::intoaV4(ip_src, buf, sizeof(buf)));
    lua_push_str_table_entry(vm, "dst", Utils::intoaV4(ip_dst, buf, sizeof(buf)));
    lua_push_uint32_table_entry(vm, "sent", v->sent);
    lua_push_uint32_table_entry(vm, "rcvd", v->rcvd);
