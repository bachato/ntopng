    ...
    switch (t) {
      case LUA_TSTRING:
        if (!strcmp(key, "sortColumn")) {
          ...
          if (sscanf(str, "%d.%d", &l7proto_filter_master_proto,
                     &l7proto_filter_app_proto) != 2)
            l7proto_filter_app_proto = atoi(str);
        }  // else
           // ntop->getTrace()->traceEvent(TRACE_ERROR, "Invalid string type
           // (%s) for option %s", lua_tostring(L, -1), key);
        break;
      ...
      case LUA_TBOOLEAN:
        ...
        else if (!strcmp(key, "periodicFlows"))
          periodic_flows = lua_toboolean(L, -1) ? 1 : 0;
        else if (!strcmp(key, "filteredFlows"))
          filtered_flows = lua_toboolean(L, -1) ? 1 : 0;
        // else
        // ntop->getTrace()->traceEvent(TRACE_ERROR, "Invalid bool type for
        // option %s", key);
        break;
