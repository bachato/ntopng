...
ParsedeBPF::ParsedeBPF(const ParsedeBPF& pe, bool swap_directions) {
  ...
  if (!swap_directions) {
    ...
    memcpy(&src_container_info, &pe.src_container_info,
           sizeof(src_container_info));
    memcpy(&dst_container_info, &pe.dst_container_info,
           sizeof(dst_container_info));
    ...
    memcpy(&src_container_info, &pe.dst_container_info,
           sizeof(src_container_info));
    memcpy(&dst_container_info, &pe.src_container_info,
           sizeof(dst_container_info));
  ...
  if (src_process_info.process_name)
    src_process_info.process_name = strdup(src_process_info.process_name);
  if (src_process_info.pkg_name)
    src_process_info.pkg_name = strdup(src_process_info.pkg_name);
  if (src_process_info.cmd_line)
    src_process_info.cmd_line = strdup(src_process_info.cmd_line);
  if (src_process_info.uid_name)
    src_process_info.uid_name = strdup(src_process_info.uid_name);
  if (src_process_info.father_process_name)
    src_process_info.father_process_name =
        strdup(src_process_info.father_process_name);
  if (src_process_info.father_pkg_name)
    src_process_info.father_pkg_name = strdup(src_process_info.father_pkg_name);
  if (src_process_info.father_uid_name)
    src_process_info.father_uid_name = strdup(src_process_info.father_uid_name);
  if (dst_process_info.process_name)
    dst_process_info.process_name = strdup(dst_process_info.process_name);
  if (dst_process_info.pkg_name)
    dst_process_info.pkg_name = strdup(dst_process_info.pkg_name);
  if (dst_process_info.cmd_line)
    dst_process_info.cmd_line = strdup(dst_process_info.cmd_line);
  if (dst_process_info.uid_name)
    dst_process_info.uid_name = strdup(dst_process_info.uid_name);
  if (dst_process_info.father_process_name)
    dst_process_info.father_process_name =
        strdup(dst_process_info.father_process_name);
  if (dst_process_info.father_pkg_name)
    dst_process_info.father_pkg_name = strdup(dst_process_info.father_pkg_name);
  if (dst_process_info.father_uid_name)
    dst_process_info.father_uid_name = strdup(dst_process_info.father_uid_name);

  container_info_set = pe.container_info_set;

  if (src_container_info.id)
    src_container_info.id = strdup(src_container_info.id);
  if (src_container_info.name)
    src_container_info.name = strdup(src_container_info.name);

  if (src_container_info.data_type == container_info_data_type_k8s) {
    if (src_container_info.data.k8s.pod)
      src_container_info.data.k8s.pod = strdup(src_container_info.data.k8s.pod);
    if (src_container_info.data.k8s.ns)
      src_container_info.data.k8s.ns = strdup(src_container_info.data.k8s.ns);
  } else if (src_container_info.data_type == container_info_data_type_docker)
    ;

  if (dst_container_info.id)
    dst_container_info.id = strdup(dst_container_info.id);
  if (dst_container_info.name)
    dst_container_info.name = strdup(dst_container_info.name);

  if (dst_container_info.data_type == container_info_data_type_k8s) {
    if (dst_container_info.data.k8s.pod)
      dst_container_info.data.k8s.pod = strdup(dst_container_info.data.k8s.pod);
    if (dst_container_info.data.k8s.ns)
      dst_container_info.data.k8s.ns = strdup(dst_container_info.data.k8s.ns);
...
ParsedeBPF::~ParsedeBPF() {
  ...
  if (src_process_info.pkg_name) free(src_process_info.pkg_name);
  if (src_process_info.cmd_line) free(src_process_info.cmd_line);
  if (src_process_info.uid_name) free(src_process_info.uid_name);
  if (src_process_info.father_process_name)
    free(src_process_info.father_process_name);
  if (src_process_info.father_pkg_name) free(src_process_info.father_pkg_name);
  ...
  if (dst_process_info.pkg_name) free(dst_process_info.pkg_name);
  if (dst_process_info.cmd_line) free(dst_process_info.cmd_line);
  if (dst_process_info.uid_name) free(dst_process_info.uid_name);
  if (dst_process_info.father_process_name)
    free(dst_process_info.father_process_name);
  if (dst_process_info.father_pkg_name) free(dst_process_info.father_pkg_name);
...
bool ParsedeBPF::update(const ParsedeBPF* const pe) {
  ...
  if (pe) {
    if (container_info_set && pe->container_info_set &&
        ((src_container_info.id && pe->src_container_info.id &&
          strcmp(src_container_info.id, pe->src_container_info.id) != 0) ||
         (dst_container_info.id && pe->dst_container_info.id &&
          strcmp(dst_container_info.id, pe->dst_container_info.id) != 0))) {
      ...
      if (!warning_shown) {
        ntop->getTrace()->traceEvent(
            TRACE_WARNING,
            "Attempting to update container info for a different container.");
        warning_shown = true;
      }
...
void ParsedeBPF::getProcessInfoJSONObject(const ProcessInfo* proc,
  ...
  json_object_object_add(
      proc_object, "NAME",
      json_object_new_string(proc->process_name ? proc->process_name : ""));
  json_object_object_add(
      proc_object, "PKG_NAME",
      json_object_new_string(proc->pkg_name ? proc->pkg_name : ""));
  json_object_object_add(
      proc_object, "CMDLINE",
      json_object_new_string(proc->cmd_line ? proc->cmd_line : ""));
  ...
  json_object_object_add(
      proc_object, "USER_NAME",
      json_object_new_string(proc->uid_name ? proc->uid_name : ""));

  if (proc->father_pid > 0) {
    json_object_object_add(proc_object, "FATHER_PID",
                           json_object_new_int64(proc->father_pid));
    json_object_object_add(
        proc_object, "FATHER_NAME",
        json_object_new_string(
            proc->father_process_name ? proc->father_process_name : ""));
    json_object_object_add(
        proc_object, "FATHER_PKG_NAME",
        json_object_new_string(proc->father_pkg_name ? proc->father_pkg_name
                                                     : ""));
    ...
    json_object_object_add(
        proc_object, "FATHER_USER_NAME",
        json_object_new_string(proc->father_uid_name ? proc->father_uid_name
                                                     : ""));
  }
}
...
void ParsedeBPF::processInfoLua(lua_State* vm, const ProcessInfo* proc) const {
  ...
  lua_push_str_table_entry(vm, "pkg_name",
                           proc->pkg_name ? proc->pkg_name : "");
  ...
  lua_push_str_table_entry(vm, "user_name",
                           proc->uid_name ? proc->uid_name : "");
  ...
  if (proc->father_pid > 0) {
    ...
    lua_push_str_table_entry(
        vm, "father_pkg_name",
        proc->father_pkg_name ? proc->father_pkg_name : "");
    lua_push_str_table_entry(
        vm, "father_user_name",
        proc->father_uid_name ? proc->father_uid_name : "");
  }
}
