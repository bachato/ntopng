...
JobQueue::~JobQueue() {
  for (std::map<u_int32_t /* id */, std::pair<FILE*, std::string> >::iterator
           it = running_jobs.begin();
       it != running_jobs.end(); ++it) {
    if (it != running_jobs.end()) fclose(it->second.first);
  }
}
...
void JobQueue::idleTask() {
  ...
  if (do_trace)
    ntop->getTrace()->traceEvent(TRACE_WARNING, "JobQueue::idleTask()");
  ...
  if (running_jobs.size() < MAX_NUM_CONCURRENT_JOBS) {
    while (!job_queue.empty()) {
      std::pair<u_int32_t /* id */, std::string /* job */> item =
          job_queue.front();
      FILE* fd = popen(item.second.c_str(), "r");

      if (do_trace)
        ntop->getTrace()->traceEvent(TRACE_WARNING, "Started %s",
                                     item.second.c_str());
      ...
      if (fd == NULL) {
        ntop->getTrace()->traceEvent(TRACE_WARNING, "Unable to run job [%s]",
                                     item.second.c_str());
        completed_jobs[item.first] =
            std::make_pair(time(NULL), std::string(""));
      } else
  ...
  for (std::map<u_int32_t /* id */, std::pair<FILE*, std::string> >::iterator
    ...
    while (true) {
      ...
      job_completed:
        if (do_trace)
          ntop->getTrace()->traceEvent(TRACE_WARNING, "[Job %d] Completed", id);
        ...
        if (l != NULL) {
          if (do_trace)
            ntop->getTrace()->traceEvent(TRACE_WARNING, "[Job %d] %s", id, l);
  ...
  if (job_completed) purgeOldResults();
...
void JobQueue::purgeOldResults() {
  if (!ntop->getGlobals()->isShutdown()) {
    ...
    for (std::map<u_int32_t /* id */, std::pair<time_t, std::string> >::iterator
             it = completed_jobs.begin();
